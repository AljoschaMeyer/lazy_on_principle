...
This design does leave open some questions: what if the \texttt{consume} function of the created consumer is called more often then there are \texttt{consume} keywords in the main consumer body? Should the main consumer body be automatically wrapped in a loop to prevent this situation? Should it always be valid to jump to the \texttt{until consume final} block, or only at the end of the main consumer body? How should the created consumer indicate violation of the resulting usage contract? We do not have immediate answers, we simply want to point out that --- after extrapolating the logic that leads to our API designs --- designing generators into languages without a corresponding consumer equivalent feels questionable.

A particular usecase we want to highlight for explicit (asynchronous) \texttt{consumer} syntax is that of implementing asynchronous decoders or parsers. Typically this involves writing a state-machine or otherwise putting a lot of manual work into ensuring a decoder that can suspend its execution when reaching the end of input and then resume once more input becomes available. The \texttt{consumer} syntax allows writing asynchronous decoders that look just like synchronous ones.

Assuming the open questions around dedicated \texttt{consumer} syntax have been solved, the next logical step is to combine the \texttt{consumer} and \texttt{producer} keywords into a more powerful \texttt{conducer} language construct. Here we sketch a decoder conducer for 16-bit integers from 8-bit integers, and an encoder conducer for 16-bit integers into 8-bit integers (any real language should allow eliding the trivial \texttt{until consume final} blocks):
...
Finally, we want to draw a parallel to coroutines\cite{moura2009revisiting}, as implemented, for example, in Lua\cite{ierusalimschy2006programming}. In (this particular brand of) coroutines, the \texttt{yield} expression in the coroutine implementation not only yields a value to the outsde world, but it also evaluates to a value that is given as part of the expression that resumes the coroutine. We can see our conducer syntax as a generalization of this pattern. Coroutines tie incoming and outgoing communication to the same points in the coroutine, marked by \texttt{yield}. Our syntax allows arbitrarily splitting the communication. By extension, conducers naturally generalize coroutines.
